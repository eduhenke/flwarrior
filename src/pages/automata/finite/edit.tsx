...
import IconBase, { PlaySquareOutlined } from "@ant-design/icons";
...
import { RulesContainer, NewTransitionModaContent } from "./MachineEditStyles";
...
export default function RegularMachineEdit(): JSX.Element {
    ...
    const convertToGrammar = async () => {
        // Convert To Grammar
        console.log('converting', { machine })
                        ...
                        <RulesContainer>
                            <StateMachineEditor
                                onUpdate={diagram => {
                                    if (!machine) return;
                                    if (
                                        JSON.stringify(diagram.states) == JSON.stringify(machine.get('states')) &&
                                        JSON.stringify(diagram.transitions) == JSON.stringify(machine.get('transitions'))
                                    ) return;
                                    const updated = machine
                                        .update('states', () => diagram.states)
                                        .update('transitions', () => diagram.transitions);
                                    console.log({ machine, diagram, updated })
                                    setMachine(updated)
                                }}
                                states={states}
